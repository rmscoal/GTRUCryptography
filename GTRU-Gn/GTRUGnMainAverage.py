...
if __name__ == '__main__':
    params = list((251,3,2521)) #nilai-nilai untuk n, p, dan q secara berurutan. 
    message = readmessage('./test_message.txt') 
    timeName = ['encryption', 'decryption', 'total', 'cpu-percent', 'memory-usage']
    timeStamp = ['miliseconds','miliseconds','seconds','%', '%']
    avgTime = [0,0,0,0,0]
    for _ in range(50):
    ...
    for i in range(len(avgTime)):
        ...
        if i <= 2:
            print(f"Average time for {timeName[i]} is {avgTime[i]} {timeStamp[i]}.")
        else:
            print(f"Resources needed for {timeName[i]} increases/decreases at {avgTime[i]} {timeStamp[i]}.")