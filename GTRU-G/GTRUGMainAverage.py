...
def main(params, message):
    ...
    mem_start = psutil.virtual_memory()[2]
    cpu_start = psutil.cpu_percent(4)

    # print(f"Parameter publik yang dipilih adalah: n={n}, p={p}, q={q}")
    m, m_add_size = custom_message_size(strToBinary(message),n)[:2]
    start_time = time.time()
    keypriv = gen_keypriv(n,p,q) 
    keypub = gen_keypub(keypriv,q)

    mem_current_1 = psutil.virtual_memory()[2]
    cpu_current_1 = psutil.cpu_percent()
    r = poly_r(n, p)

    start_time_enkripsi = time.time()
    e = enkripsi(keypub,m,r,q)
    end_time_enkripsi = (time.time() - start_time_enkripsi)*1000
    mem_current_2 = psutil.virtual_memory()[2]
    cpu_current_2 = psutil.cpu_percent()
    # print(f"cipherteks: \n {e}")
    ...
    end_time_dekripsi = (time.time() - start_time_dekripsi)*1000
    mem_current_3 = psutil.virtual_memory()[2]
    cpu_current_3 = psutil.cpu_percent()

    end_time_total = time.time() - start_time
    d = bintostr(d)
    mem_end = (mem_current_1 + mem_current_2 + mem_current_3)/3 - mem_start
    cpu_end = (cpu_current_1 + cpu_current_2 + cpu_current_3)/3 - cpu_start
    # print(f"hasil dekripsi:\n{d}")
    # print("--- waktu dekripsi : %s milidetik ---" % 
    #     ((time.time() - start_time_dekripsi)*1000))

    # print("\n--- waktu total (temasuk pembangunan kunci) : %s detik ---" % 
    #     (time.time() - start_time))
    return end_time_enkripsi, end_time_dekripsi, end_time_total, cpu_end, mem_end

if __name__ == '__main__':
    params = list((467,3,3739)) #nilai-nilai untuk n, p, dan q secara berurutan. 
    message = readmessage('./test_message.txt') 
    timeName = ['enkripsi', 'dekripsi', 'total', 'cpu-percent', 'memory-usage']
    timeStamp = ['milidetik','milidetik','detik','persen', 'persen']
    avgTime = [0,0,0,0,0]
    for _ in range(50):
        res = main(params, message)
        for index, timeValue in enumerate(res):
            avgTime[index] += timeValue
    for i in range(len(avgTime)):
        avgTime[i] = round(avgTime[i] / 50, 3)
        if i <= 2:
            print(f"Rata-rata waktu untuk {timeName[i]} adalah {avgTime[i]} {timeStamp[i]}.")
        else:
            print(f"Sumber daya yang dibutuhkan untuk {timeName[i]} adalah {avgTime[i]} {timeStamp[i]}.")